...
[![DeepWiki](https://img.shields.io/badge/DeepWiki-mpp--swift-blue.svg)](https://deepwiki.com/amitach/mpp-swift)
...
- **`MPPDiscovery`** - OpenAPI `x-payment-info` discovery: parse, generate, validate.
...
| `MPPClient` | available | The 402 client flow (send → parse → select → build → retry) over `URLSessionTransport` (with redirect-downgrade protection); plus the vendor-neutral `PaymentAuthorizer` consent seam (the protocol + `AllowAllAuthorizer` + `SpendingCapAuthorizer`) consulted once at the credential-build chokepoint, shared by the HTTP and MCP clients |
