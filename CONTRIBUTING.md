...
2. **No flaky tests.** Inject the clock (no `Date()` in testable paths), stub the network in unit tests, seed randomness, never `sleep`, isolate state per test, assert byte-exact via canonical encoding. A flake that slips through surfaces on its own CI run; fix the root cause rather than retrying it away.
