...
/// `transferWithMemo` transaction with the V2 keychain signature, attaching the key
/// authorization on the provisioning charge. The same fixed inputs as the Rust test
/// (rust/tempo-tx-ffi/src/lib.rs: `subscription_charge_attaches_key_authorization`), so a
...
struct FFISubscriptionChargeTxBuilderTests {
    ...
    // The byte-exact no-auth charge tx (V2 keychain signature), shared with the Rust
    // GOLDEN_SUBSCRIPTION_CHARGE_TX: same access key, payer (root), currency, recipient, amount,
    // memo, nonce, and fee. The trailing b856 04 <payer> <65-byte sig> is the keychain signature.
    private static let goldenCharge =
        "76f8f082a5bf830f4240843b9aca00830186a0f87ef87c9420c0000000000000000000000000000000" +
        ...
        "ababababababababababababababababababababababc0800780808080c0b856047e5f4552091a69125d" +
        "5dfcb7b8c2659029395bdf34b62d4b8e525ea50f6941cd3e0b13750c90eb7098290614fe734921ead18c" +
        "4c471151d237efd57add8ead8e52534aa2f968307173523013f281b0236d3056e81b"
    ...
    @Test("builds the byte-exact golden 0x76 keychain-signed transferWithMemo charge (no auth)")
    ...
    @Test("reads the nonce for the payer (root) account the charge executes for")
