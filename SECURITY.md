...
- **Transport (§11.2):** TLS required; non-`https` rejected at runtime (minimum TLS 1.2, default 1.3). A scoped `allowInsecureLocal` opt-in permits loopback only, for tests and the conformance harness; the test suite rejects a non-loopback server over plain HTTP. The same gate guards the proxy's *upstream*: `ProxyService.init` rejects a non-loopback `http` `baseURL` (which would ship the proxy's injected upstream credentials in plaintext), allowing a loopback `http` origin only under the explicit `allowInsecureLocal` opt-in.
...
- **Replay (§11.3):** single-use proof semantics via an atomic replay store, consumed before any side effect; unpaid requests perform no work.
- **Idempotency (§11.4):** an optional `IdempotencyStore` seam (default `InMemoryIdempotencyStore`, an actor) keyed on the request `Idempotency-Key` header: a settled response is recorded and replayed on retry (so a lost-response retry is not re-charged on a now-spent challenge), a key held by an in-flight request answers `409 Conflict`, and an unsettled attempt (a `402`, or a transport guard) releases the key so a genuine retry can still pay. A TTL-bounded backend (SQLite/Redis) plugs the same seam.
...
- **Caching (§11.10) and DoS (§11.12):** `Cache-Control: no-store` on 402 and `private` on a receipted 200; a request body cap (HTTP 413 over a fixed limit); an optional per-client rate-limiter seam (`RateLimiter`, default `InMemoryRateLimiter` token bucket) consulted before any payment work, answering `429` + `Retry-After` when over budget and emitting a `rateLimited` event. The limiter is keyed on a host-supplied client key (the transport remote address is not in `HTTPRequest`); a shared-backend limiter (Redis) plugs the same seam, and per-deployment throttling at a reverse proxy or gateway remains available on top.
- **HTML payment page (opt-in):** the optional `ChallengePresenter` seam (`MPPHTMLServer.PaymentPagePresenter`) renders a `402` as an HTML page only for `Accept: text/html`; a non-browser client is unaffected and still gets the problem document, and the page keeps the `WWW-Authenticate` challenge and `no-store` floor. Every challenge-, config-, or theme-derived value is escaped at its boundary (`sanitizeHTML` for text/attributes, `sanitizeCSS` closing a `</style>` breakout, `<` in the embedded JSON closing a `</script>` breakout); the method-supplied `content` is a documented trusted raw inclusion. The browser service worker submits the credential over a same-origin navigation only, never persists it (cleared after one use), and self-unregisters, so the credential never rides in a URL or form field.
...
The Tempo `0x76` transactions (the channel open/topUp/close, and the recurring subscription charge) are built by `rust/tempo-tx-ffi`, a Rust crate that binds Tempo's own `tempo-primitives` (the chain's canonical transaction format). Unlike the npm tooling below, **this is a build input that ships in the artifact**, so it is a higher-trust surface and carries its own controls:
...
- **The published Apple artifact is checksum-pinned.** The release pipeline (`.github/workflows/release-ffi.yml`, triggered by a `tempo-tx-ffi-v*` tag) builds the xcframework from pinned source and publishes it as a GitHub-release asset; `Package.swift` references it by `url` **and SHA-256 `checksum`**, so SwiftPM rejects a tampered or substituted download. The default Apple build links this published artifact (currently `tempo-tx-ffi-v0.0.7`); any change to the Rust source or its FFI signature requires cutting a new release and bumping the `url` + `checksum`, so published and source never drift. The from-source `MPP_TEMPO_FFI` path (no opaque binary consumed) is how dev and CI exercise the actual build, and Linux always builds from source.
...
- **Byte-exact golden tests** lock the transaction output (deterministic RFC-6979 signing) and detect any change from a format update, a dependency bump, or our code: the channel close/open/topUp transactions, and both the unsponsored and the **fee-payer-sponsored** subscription charge, are each byte-locked; the live-Moderato test is the authoritative on-chain check. No transaction encoding is hand-rolled.
- **Key hygiene:** every raw private key the shim handles (the channel signer, the per-charge subscription **access key**, and the optional **fee-payer / gas-sponsor key**) is zeroized after use, and the `k256` signing keys are themselves zeroize-on-drop. The discipline is path-complete: each key is wiped **before any fallible step** (so an early error return cannot leave key bytes on the stack), in **every stack frame** that holds a copy (a `[u8; 32]` is `Copy`, so a callee zeroizing its own copy does not absolve the caller), and the FFI boundary keeps decoded keys in a `Zeroizing` wrapper and hands callees only a copy. Keys are **borrowed, never moved out of their `Option`** before wiping, because moving a `Copy` array out leaves the original bytes behind (the `None` write rewrites only the discriminant); the in-place `zeroize` overwrites the real bytes.
